...
class ModelWrapper(nn.Module):
    ...
    def get_batch_coords(self, x=None):
        ...
        coords = (
            coords.clone()
            .detach()[None, ...]
            .repeat((meta_batch_size,) + (1,) * len(coords.shape))
        )
    ...
    def gradncp(self, x):
        ...
        self.gradncp_index = torch.sort(gradient_norm, dim=1, descending=True)[1][
            :, : int(coords_len * ratio)
        ]
        self.gradncp_coord = torch.gather(
            coords,
            1,
            self.gradncp_index.unsqueeze(dim=2).repeat(1, 1, self.args.in_size),
        )
        self.gradncp_index = self.gradncp_index.unsqueeze(dim=1).repeat(
            1, self.args.out_size, 1
        )
    ...
    def forward(self, x=None, conditions=None):
        if self.data_type == "img":
            return self.forward_img(x, conditions)
    ...
    def forward_img(self, x, conditions):
        coords, meta_batch_size = self.get_batch_coords(x)
        coords = coords.to(self.args.device)
        #         print(coords.shape)

        #         print("conditions in the forward_img: ", conditions.shape)
        #         print("x esistence: ", exists(x))

        #         print("coords: ", coords.shape)
        out = self.model(coords, conditions)
        ...
        if exists(x):
            if self.sampled_coord is None and self.gradncp_coord is None:
                return F.mse_loss(
                    x.view(meta_batch_size, -1),
                    out.reshape(meta_batch_size, -1),
                    reduce=False,
                ).mean(dim=1)
            elif self.gradncp_coord is not None:
                x = rearrange(x, "b c h w -> b c (h w)")
    ...
    def forward_img3d(self, x):
        ...
        out = rearrange(
            out, "b c (h w d) -> b c h w d", h=self.height, w=self.width, d=self.depth
        )
        return out
