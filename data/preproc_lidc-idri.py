...
def preprocess_nifti(input_path, output_path):
    ...
    assert out_normalized.shape == (
        ...
        256,
        256,
    ), "The output shape should be (320,320,320)"
...
if __name__ == "__main__":
    ...
    for patient in os.listdir(args.dicom_dir):
        ...
        dicom2nifti.convert_directory(
            os.path.join(args.dicom_dir, patient), os.path.join(args.nifti_dir, patient)
        )
    ...
    for root, dirs, files in os.walk(args.nifti_dir):
        for file in files:
            try:
                preprocess_nifti(
                    os.path.join(root, file), os.path.join(root, "processed.nii.gz")
                )
            except:
