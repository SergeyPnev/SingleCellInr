...
def test_model(args, model_wrapper, test_loader, logger=None):
    ...
    for n, data in enumerate(tqdm(test_loader)):
        ...
        if args.data_type == "img":
            ...
            ssim_results = ssim(pred, data, data_range=1.0).mean()
        ...
        if n % 10 == 0:
            ...
            log_(
                f"*[EVAL {n}][PSNR %.6f][LPIPS %.6f][SSIM %.6f][MSE %.6f]"
                % (
                    metric_logger.psnr.global_avg,
                    metric_logger.lpips.global_avg,
                    metric_logger.ssim.global_avg,
                    metric_logger.mse.global_avg,
                )
            )
    ...
    metric_logger.synchronize_between_processes()
    log_(
        f"*[EVAL Final][PSNR %.8f][LPIPS %.8f][SSIM %.8f][MSE %.8f]"
        % (
            metric_logger.psnr.global_avg,
            metric_logger.lpips.global_avg,
            metric_logger.ssim.global_avg,
