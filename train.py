...
def main(args):
    ...
    infinite_sampler = InfiniteSampler(
        train_set, rank=0, num_replicas=1, shuffle=True, seed=args.seed
    )
    ...
    test_loader = DataLoader(
        test_set, batch_size=args.test_batch_size, shuffle=False, num_workers=4, drop_last=True
    )
    ...
    model.modulations = torch.zeros(
        size=[args.batch_size, args.latent_modulation_dim], requires_grad=True
    ).to(device)
    ...
    """ Define logger """
    fname = (
        f"{args.save_dir}/{args.dataset}_bs{args.batch_size}_inner{args.inner_steps}_size{args.img_size}_gamma{args.data_ratio}_"
