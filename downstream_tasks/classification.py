...
class EfficientNetB0Classifier(nn.Module):
    def __init__(self, num_classes, mode="rgb"):
        ...
        self.efficientnet_b0.classifier[1] = nn.Linear(
            self.efficientnet_b0.classifier[1].in_features, num_classes
        )
        if mode == "grayscale":
...
def main(args):
    ...
    if args.classifier == "simple":
        ...
        train_set = PneumoniaMNIST(
            split="train", transform=transforms, download="True", size=64
        )
        val_set = PneumoniaMNIST(
            split="val", transform=transforms, download="True", size=64
        )
        test_set = PneumoniaMNIST(
            split="test", transform=transforms, download="True", size=64
        )
    else:
