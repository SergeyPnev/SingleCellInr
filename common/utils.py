...
def save_checkpoint(
    args, step, best_psnr, model, optim_state, logdir, is_best=False, suffix=""
):
    if is_best:
        prefix = "best"
    else:
...
def resume_training(args, model, optimizer):
    if args.resume_path is not None:
        model_state, optimizer_state, config, lr_dict = load_checkpoint(
            args.resume_path, mode="best"
        )
    ...
    def log_image(self, tag, images, step):
        ...
        if self.rank == 0:
            ...
            if len(images.shape) == 4:  # 2D Images
                self.writer.add_images(tag, images[:, :3, ...], step)
            ...
            if len(images.shape) == 5:  # 3D Images
                ...
                slices_depth = images[
                    :, :, middle_depth, :, :
                ]  # Middle slice along depth
                slices_height = images[
                    :, :, :, middle_height, :
                ]  # Middle slice along height
                slices_width = images[
                    :, :, :, :, middle_width
                ]  # Middle slice along width
